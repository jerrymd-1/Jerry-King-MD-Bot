 ...
 * MODIFIED: Better button detection for menu
 */
...
const handleMessage = async (sock, msg) => {
  try {
    ...
    // ==================== FIXED: BUTTON DISPATCH WITH MENU SUPPORT ====================
    // Check for button response in multiple locations
    let isButtonHandled = false;
    
    // Get button ID from various sources
    let buttonId = null;
    ...
    // If it's a menu button, handle it directly
    if (buttonId && buttonId.startsWith('menu_cat_')) {
      console.log('[HANDLER] Menu button clicked:', buttonId);
      
      const menuCommand = commands.get('menu');
      if (menuCommand && typeof menuCommand.handleButtonResponse === 'function') {
        const extra = {
          from,
          sender,
          isGroup,
        ...
        await menuCommand.handleButtonResponse(sock, msg, extra);
        isButtonHandled = true;
      }
    }
    
    // If button wasn't handled by menu, try other commands
    if (!isButtonHandled) {
      const buttonPayload = msg.message?.buttonsResponseMessage || msg.message?.interactiveResponseMessage;
      if (buttonPayload) {
        for (const command of new Set(commands.values())) {
